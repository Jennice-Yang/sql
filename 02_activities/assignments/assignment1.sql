...
WHERE productID = 4 or ProductID = 9;

-- option 2
SELECT product_id, customer_id
FROM customer_purchases
WHERE productID_IN (4,9);
...
*/
-- option 1
SELECT product_id, vendor_id, customer_id, quantity, cost_to_customer_per_qty, 
(quantity * cost_to_customer_per_qty) AS price
FROM Customer_purchases
WHERE vendor_id >= 8 AND vendor_id <= 10; 

-- option 2
SELECT purchase_id, customer_id, product_id, vendor_id, quantity, cost_to_customer_per_qty, 
(quantity * cost_to_customer_per_qty) AS price
FROM customer_purchases
...
HAVING (cp.quantity * cp.cost_to_customer_per_qty) AS price 
	SUM (Price) > 2000
...
JOIN market_date_info md ON cp.market_date = md.market_date,
